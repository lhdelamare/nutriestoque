...
async function main() {
  console.log('⚡ Criando tabelas MySQL no banco nutri_estoque...');

  await prisma.$executeRawUnsafe('SET FOREIGN_KEY_CHECKS = 0');

  // Drop tables if existing
  const drops = ['user', 'fractionedlabel', 'dispatch', 'loss', 'batch', 'purchase', 'product', 'category', 'supplier', 'requester', 'department'];
  for (const t of drops) {
    try { await prisma.$executeRawUnsafe(`DROP TABLE IF EXISTS \`${t}\``); } catch (e) {}
  }
  ...
  console.log('🎉 100% SUCESSO! Banco de Dados MySQL local criado e populado!');
  await prisma.$disconnect();
}

main().catch((err) => {
  console.error('❌ Erro na criação de tabelas:', err);
  process.exit(1);
});
